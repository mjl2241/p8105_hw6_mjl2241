...
### Problem 2
Import data & load and clean the data for regression analysis (i.e. convert numeric to factor where appropriate, check for missing data, etc.).
``` {r import dataset}
bw=
...
step.model = stepAIC(full.model, direction = "both", 
                      trace = FALSE)
```
The result indicated that 13 variables, including babysex2 (female), bhead, blength, delwt, fincome, gaweeks, mheight, mrace2(black), mrace3(Asian), mrace4(Puerto Rian), parity, ppwt, and smoken were significant.  predictors. 
```{r}
#Final model:
final_model = 
...
```
The output shows that R^2 is 0.71, indicating that these 14 predictors explain 71% of the variance in birthweight.And now onto plotting of model residuals against fitted values – using add_predictions and add_residuals in making this plot.
```{r}
modelr::add_residuals(bw, final_model)

bw %>% 
  add_residuals(model = final_model, var = "resid") %>% 
  add_predictions(model = final_model, var = "pred") %>% 
